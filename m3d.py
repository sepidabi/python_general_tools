...
class m3d:
    ...
    def readpar(self):
        ...
        # cont info
        self.cont = [ mc.Cont() for i in range(self.atom.ncont)]
        ...
        #line info
        self.line = [ mc.Line() for i in range(self.atom.nline)]
        for l in self.line:
    ...
    def readn(self):
        ...
        self.atom.n = np.memmap(fname, dtype='float32', mode='r', shape=(nx,ny,nz,nlevel),order='F')
        self.atom.nstar = np.memmap(fname, dtype='float32', mode='r', shape=(nx,ny,nz,nlevel), offset=gs, order='F')
        
        self.atom.ntot = np.memmap(fname, dtype='float32', mode='r', shape=(nx,ny,nz) ,offset=gs*2, order='F' )
    ...
    def readatmos(self):
        ...
        self.atmos.ne  = np.memmap(fname, dtype='float32', mode='r', shape=s, order='F')
        self.atmos.tg  = np.memmap(fname, dtype='float32', mode='r', shape=s, offset=gs, order='F')
        self.atmos.vx  = np.memmap(fname, dtype='float32', mode='r', shape=s ,offset=gs*2, order='F' )
        self.atmos.vy  = np.memmap(fname, dtype='float32', mode='r',  shape=s ,offset=gs*3, order='F' )
        self.atmos.vz  = np.memmap(fname, dtype='float32', mode='r',  shape=s ,offset=gs*4, order='F' )
        self.atmos.rho = np.memmap(fname, dtype='float32', mode='r',  shape=s ,offset=gs*5, order='F' )
        self.atmos.nh  = np.memmap(fname, dtype='float32', mode='r',  shape=(nx,ny,nz,nhl) ,offset=gs*6, order='F' )
        self.atmos.vturb = np.memmap(fname, dtype='float32', mode='r', shape=s ,offset=gs*12, order='F' )
   ...
   ####################################################################
    ...
    def readrtq(self):
        ...
        self.atmos.x500  = np.memmap(fname, dtype='float32', mode='r', shape=s, order='F')
        self.atom.dopfac  = np.memmap(fname, dtype='float32', mode='r', shape=s, offset=gs, order='F')
        i=2
        for l in self.line:
            l.adamp = np.memmap(fname, dtype='float32', mode='r', shape=s ,offset=gs*i, order='F' )
            i+=1
    ...
    def readvar(self, var, all=False):
        ...
        if var in ('ie','zt1','qe','ue','ve'):
            if all:
                ...
                offset = ( 4 * sg.nx * sg.ny * np.where(self.outff == self.d.ired)[0] )
            else:
                shape = (sg.nx, sg.ny)
                offset = ( 4 * sg.nx * sg.ny * np.where(self.outff == self.d.ff)[0] )
            ...
            if all:
                ...
                offset = ( 4 * sg.nx * sg.ny * sg.nz * np.where(self.outff == self.d.ired)[0] )
            else:
                shape = (sg.nx, sg.ny, sg.nz)
                offset = ( 4 * sg.nx * sg.ny * sg.nz * np.where(self.outff == self.d.ff)[0] )

        thedata = np.memmap(fname,dtype='float32', mode='r', shape=shape,order='F',offset=offset)
