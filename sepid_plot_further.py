...
if plot_mode==2:
    ...
    axHisty.hist(y, bins=ndep, orientation='horizontal', color = 'steelBlue')
...
if plot_mode == 3:
    ...
    xlabel = r' $\rm \delta\overline{\rm I}_{Ca k}$ [%]'
    ylabel = r' $\rm \delta\overline{\rm I}_{H_{\alpha}}$ [%]'
        
    a = sns.jointplot(x, y, kind='reg', color = 'red',scatter_kws={'alpha': 0.5}, xlim = [-3, 40], ylim = [-10,40])
    a.set_axis_labels(xlabel,ylabel)
    #a.plot_joint(axhline(0, color = 'black', alpha = 0.8, linestyle = '--', linewidth = 0.5))
    #a.plot_joint(plt.scatter(true_ck, true_h, color = 'white', edgecolor = 'white'))
    #a.plot_joint(plt.scatter(true_ck, true_h, color = 'darkgray', edgecolor = 'dimgray',alpha = 0.7))
...
if plot_mode==4:
    ...
    true_ck = I_dif_true[np.where(t_dif_true<=0)[0]]
    true_h = t_dif_true[np.where(t_dif_true>0)[0]]
    
    sns.set_style("ticks")
    xlabel = r' $\rm \delta\overline{\rm I}_{Ca k}$ [10$^{-7}$ erg cm$^{-2}$ s$^{-1}$ Hz$^{-1}$ ster$^{-1}$]'
    ylabel = r' $\rm \delta$T [K]'
    a = sns.jointplot(x, y, kind='reg', color = 'red')
    a.set_axis_labels(xlabel,ylabel)
...
#to plot v_los
#with sns.color_palette("RdPu",ndep):
