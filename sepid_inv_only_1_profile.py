...
#inversion results
f_i = sp.profile(resdir+'fb/f'+mode+'_observed_'+index+'.nc')
f_o = sp.profile(resdir+'fb/f'+mode+'_synthetic_cycle'+cyc+'_'+index+'.nc')
f_m = sp.model(resdir+'fb/f'+mode+'_atmosout_cycle'+cyc+'_'+index+'.nc')
b_i = sp.profile(resdir+'bg/b'+mode+'_observed_'+index+'.nc')
b_o = sp.profile(resdir+'bg/b'+mode+'_synthetic_cycle'+cyc+'_'+index+'.nc')
b_m = sp.model(resdir+'bg/b'+mode+'_atmosout_cycle'+cyc+'_'+index+'.nc')
...
#for fn in range(10):
    ...
    ##========
    # inversion
    norm_obs_f = np.max(np.mean(f_i.dat[0,0,:,:,ss] ,axis = 0))
    norm_syn_f = np.max(np.mean(f_o.dat[0,0,:,:,ss] ,axis = 0))
    norm_obs_b = np.max(np.mean(b_i.dat[0,0,:,:,ss] ,axis = 0))
    norm_syn_b = np.max(np.mean(b_o.dat[0,0,:,:,ss] ,axis = 0))
    f_obs = 100*f_i.dat[0,0,:,:,ss]/norm_obs_f
    f_syn = 100*f_o.dat[0,0,:,:,ss]/norm_syn_f
    f_wav = f_i.wav
    f_n = fnpix
    ...
    #===========
    #inversion
    b_obs = 100*b_i.dat[0,0,:,:,ss]/norm_obs_b
    b_syn = 100*b_o.dat[0,0,:,:,ss]/norm_syn_b
    b_wav = b_i.wav
    b_n = bnpix
    ...
    for id in range(len(indxs)):
        if indxs[id] == index:
            ...
            if plot_mode==1:
                ...
                ax3 = plt.subplot2grid((3,6), (2,2), colspan=2, rowspan=1)
                ax4 = plt.subplot2grid((3,6), (2,4), colspan=2, rowspan=1)
                ...
                for bb in range(b_n):
                    ax2.plot(b_m.temp[0,0,bb,:].squeeze()*1e-3, b_dep, 'k-',color = 'gray',alpha = alp)
                    ax3.plot(b_m.vturb[0,0,bb,:].squeeze()*1.e-5, b_dep, 'k-',color = 'gray',alpha = alp)
                    ax4.plot(b_m.vlos[0,0,bb,:].squeeze()*1.e-5, b_dep, 'k-',color = 'gray',alpha = alp)
                #for dd in range(45):
                b_temp_m = np.mean(b_m.temp[0,0,:,:], axis = 0)
                b_temp_med = np.median(b_m.temp[0,0,:,:], axis = 0)
                b_los_m = np.mean(b_m.vlos[0,0,:,:], axis = 0)
                b_los_med = np.median(b_m.vlos[0,0,:,:], axis = 0)
                b_turb_m = np.mean(b_m.vturb[0,0,:,:], axis = 0)
                b_turb_med = np.median(b_m.vturb[0,0,:,:], axis = 0)
                    
                for ff in range(f_n):
                    #f_temp_m += f_m.temp[0,0,ff,:]
                    ax2.plot(f_m.temp[0,0,ff,:].squeeze()*1e-3,f_dep,  'k-',color = 'red',alpha = alp)
                    ax3.plot(f_m.vturb[0,0,ff,:].squeeze()*1.e-5, f_dep, 'k-',color = 'red',alpha = alp)
                    ax4.plot( f_m.vlos[0,0,ff,:].squeeze()*1.e-5, f_dep,'k-',color = 'red',alpha = alp)
                #for dd in range(45):
                f_temp_m = np.mean(f_m.temp[0,0,:,:], axis = 0)
                f_temp_med = np.median(f_m.temp[0,0,:,:], axis = 0)
                f_los_m = np.mean(f_m.vlos[0,0,:,:], axis = 0)
                f_los_med = np.median(f_m.vlos[0,0,:,:], axis = 0)
                f_turb_m = np.mean(f_m.vturb[0,0,:,:], axis = 0)
                f_turb_med = np.median(f_m.vturb[0,0,:,:], axis = 0)
                ...
                for n in range(len(nodes_vlos)):
                    ...
                    ax11.set_ylabel(r'I/I$_{\mathrm{cont}}$ [%]', fontdict = font)
                    ax11.set_xlabel(r'Wavelength [$\mathrm{\AA}$]', fontdict = font)
                    ...
                    ax12.set_xlabel(r'Wavelength [$\mathrm{\AA}$]', fontdict = font)
                    ...
                    ax13.set_xlabel(r'Wavelength [$\mathrm{\AA}$]', fontdict = font)
                ...
                ax3.set_xlabel(r'v$_{\mathrm{turb}}$ [km/s]', fontdict = font)
                ax3.tick_params(axis='both', which='major', labelsize=8)
                                
                ax4.set_xlim(-6,7.5)
                ax4.set_ylim(0.1,-6.3)
                #ax4.set_ylabel(r'log($\mathrm{\tau}$)', fontdict = font)
                ax4.set_xlabel(r'v$\mathrm{_{LOS}}$ [km/s]', fontdict = font)
