...
# DECLERATIONS
indx_case = 0
fibril_indxs = ['06_165307'] # ['06_165307', '06_165307', '06_165307', '06_165307']
bg_indxs =  ['06_165404'] # ['06_165404', '06_165404', '06_165404', '06_165404']
...
alp = 0.9 #transparency of the plotted lines
alp2 = 0.9
    ...
    for id in range(len(indxs)):
        if indxs[id] == index:
            ...
            for jj in range(fnpix):
                ...
                linewidth = 0.75
                markersize_i = 3
                ax11.plot(b_obs[jj,0:42],'.',color = 'gray',markersize = 4.5, alpha = alp2)
                ax11.plot(f_obs[jj,0:42],'.',color = 'red',markersize = 4.5, alpha = alp2)
                ax11.plot(b_syn[jj,0:42], color='gray',linewidth = linewidth, alpha = alp2, linestyle = line_i)
                ax11.plot(f_syn[jj,0:42], color='red',linewidth = linewidth, alpha = alp2, linestyle = line_i)
                
                ax12.plot(b_obs[jj,43:129],'.',color = 'gray',markersize = 4.5)
                ax12.plot(f_obs[jj,43:129],'.',color = 'red',markersize = 4.5)
                ax12.plot(b_syn[jj,43:129], color='gray',linewidth = linewidth, alpha = alp2, linestyle = line_i)
                ax12.plot(f_syn[jj,43:129], color='r',linewidth = linewidth, alpha = alp2, linestyle = line_i)
                
                ax13.plot(b_obs[jj,130:],'.',color = 'gray',markersize = 4.5, label = 'b$\mathrm{_{obs}}$')
                ax13.plot(f_obs[jj,130:],'.',color = 'red',markersize = 4.5, label = 'f$\mathrm{_{obs}}$')
                ax13.plot(b_syn[jj,130:], color='gray',linewidth = linewidth, label = 'b$\mathrm{_{syn}}$', alpha = alp2, linestyle = line_i)
                ax13.plot(f_syn[jj,130:], color='r',linewidth = linewidth, label = 'f$\mathrm{_{syn}}$', alpha = alp2, linestyle = line_i)
                
                
                #for bb in range(0,b_n):
                ax2.plot(b_m.temp[0,0,jj+binit,:].squeeze()*1e-3, b_dep, 'k-',color = 'gray',alpha = alp)
                ax3.plot(b_m.vturb[0,0,jj+binit,:].squeeze()*1.e-5, b_dep, 'k-',color = 'gray',alpha = alp)
                ax4.plot(b_m.vlos[0,0,jj+binit,:].squeeze()*1.e-5, b_dep, 'k-',color = 'gray',alpha = alp)
                #for ff in range(0,f_n):
                #f_temp_m += f_m.temp[0,0,finit+ff,:]
                ax2.plot(f_m.temp[0,0,finit+jj,:].squeeze()*1e-3,f_dep,  'k-',color = 'red',alpha = alp)
                ax3.plot(f_m.vturb[0,0,finit+jj,:].squeeze()*1.e-5, f_dep, 'k-',color = 'red',alpha = alp)
                ax4.plot( f_m.vlos[0,0,finit+jj,:].squeeze()*1.e-5, f_dep,'k-',color = 'red',alpha = alp)
                ...
                ax11.set_ylim(5,np.max(f_obs[:,0:42]))
                ax11.set_xlim(0,41)
                ...
                ax12.set_ylim(10, np.max(f_obs[:,43:129]))
                ...
                #for ii in range(gg):
                ax5.plot(b_x_pts[jj]-xmin,b_y_pts[jj]-ymin, marker = '+', color = 'gray',markersize = 8, alpha = 0.5)
                ax5.plot(f_x_pts[jj]-xmin,f_y_pts[jj]-ymin, marker = '+', color = 'red',markersize = 8, alpha = 0.5)
                ax6.plot(b_x_pts[jj]-xmin,b_y_pts[jj]-ymin, marker = '+', color = 'gray',markersize = 8,markerfacecolor = 'none')
                ax6.plot(f_x_pts[jj]-xmin,f_y_pts[jj]-ymin, marker = '+', color = 'red',markersize = 8,markerfacecolor = 'none')
                    
                f.set_tight_layout(True)
                plt.ion()
                #plt.show()
                outname = outdir+'profile_fr'+str(jj).zfill(4)+'.png'
                f.savefig(outname, dpi = 1000)
...
os.system("ffmpeg -r 4 -i "+outdir+'profile_fr'+"%04d.png -vcodec mpeg4 -y "+outdir+"fibril.mp4")
