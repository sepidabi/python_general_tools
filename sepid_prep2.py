...
if __name__ == "__main__":
    ...
    #making a big array to stack all the fibrils -> fe
    file_fe = sorted(file_search(savedir, fib_bg+'_obs6302_*.fits'))
    file_ca8 = sorted(file_search(savedir, fib_bg+'_obs8542_*.fits'))
    file_ck = sorted(file_search(savedir, fib_bg+'_obs3950_*.fits'))
    ntot = 0
    for nn in range(len(file_fe)):
        ntot = ntot + (mf.readfits(savedir + file_fe[nn])).shape[2]
    d1 = mf.readfits(savedir + file_fe[0]).shape[0]

    #Fe line
    d2_fe = mf.readfits(savedir + file_fe[0]).shape[1]
    fe = np.zeros((d1, d2_fe, ntot))
    init = 0
    for fn in range(len(file_fe)):
        fibril = mf.readfits(savedir + file_fe[fn])
        npix = fibril.shape[2]
        fe[:,:,init:init+npix] = fibril
        init = init+npix

    #Ca8 line
    d2_ca8 = mf.readfits(savedir + file_ca8[0]).shape[1]
    ca8 = np.zeros((d1, d2_ca8, ntot))
    init = 0
    for cn in range(len(file_ca8)):
        fibril = mf.readfits(savedir + file_ca8[cn])
        npix = fibril.shape[2]
        ca8[:,:,init:init+npix] = fibril
        init = init+npix

    #Ck line
    d2_ck = mf.readfits(savedir + file_ck[0]).shape[1]
    ck = np.zeros((d1, d2_ck, ntot))
    init = 0
    for cn in range(len(file_ck)):
        fibril = mf.readfits(savedir + file_ck[cn])
        npix = fibril.shape[2]
        ck[:,:,init:init+npix] = fibril
        init = init+npix

        
    #fe  = mf.readfits(savedir+fib_bg+'_obs6302.fits')
    #ca8= mf.readfits(savedir+fib_bg+'_obs8542.fits')
    #ck  = mf.readfits(savedir+fib_bg+'_obs3950.fits')
    ...
    wfe1, ife1 = findgrid(fe[0,0:9,0], (fe[0,4,0]-fe[0,3,0])*0.25, extra=8)
    wfe2, ife2 = findgrid(fe[0,9::,0],  (fe[0,4,0]-fe[0,3,0])*0.25, extra=12)
    wfe = np.append(wfe1,wfe2) #stacks the Fe I pair wavelength and indexes together
    ...
    wc8, ic8 = findgrid(ca8[0,:,0], (ca8[0,10,0]-ca8[0,9,0])*0.5, extra=8)
    ...
    wck, ick = findgrid(ck[0,0:21,0], (ck[0,10,0]-ck[0,9,0]), extra=8)
    ...
    fe_1 = sp.profile(nx=fe.shape[2], ny=1, ns=4, nw=wfe.size)
    ca_8 = sp.profile(nx=ca8.shape[2], ny=1, ns=4, nw=wc8.size)
    ca_k = sp.profile(nx=ck.shape[2], ny=1, ns=4, nw=wck.size+1) # add 1 continuum point!
    ...
    ca_8.wav[:] = wc8[:]
    ca_k.wav[0:-1] = wck[:]
    ca_k.wav[-1]    = ck[0,-1,0] # Continuum point
    ...
    for ii in range(4): # Fill all stokes parameters for 8542 and 6302
        fe_1.dat[0,0,:,ife,ii] = fe[ii+1,:,:] /cont[2] 
        ca_8.dat[0,0,:,ic8,ii] = ca8[ii+1,:,:] / cont[1]

        
    # Only fill Stokes I for Ca II K
    ca_k.dat[0,0,:,ick,0] = ck[1,0:21,:] / cont[0]
    ca_k.dat[0,0,:,-1,0] = ck[1,21,:]  / cont[0]
    ...
    sp_all.write(fib_bg+mod+'_observed.nc')
    ...
    print(" ")
    print("# I/O options and observation details")
    print("input_model =  "+fib_bg+mod+"_modelin.nc")
    print("input_profiles = "+fib_bg+mod+"_observed.nc")
    print("output_profiles = "+fib_bg+mod+"_synthetic_cycle1.nc")
    print("output_model = "+fib_bg+mod+"_modelout_cycle1.nc")
    print("output_atmos = "+fib_bg+mod+"_atmosout_cycle1.nc")

    
    lab = "region = {0:10.5f}, {1:8.5f}, {2:3d}, {3:e}, {4}"
    print(" ")
    print("# Regions information, they will be appended one after the other")
    print("#lines = all")
    ...
    # Fill in the model
    m = sp.model(nx=fe.shape[2], ny=1, nt=1, ndep=ntau)
    ...
    # Write to HD
    m.write(fib_bg+mod+'_modelin.nc', write_all=True)
