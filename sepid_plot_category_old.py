...
# DECLERATIONS
cat_indx = [182,190,202,196] # fibril count index in crispex
...
# Graphics
plt.close('all')
f = plt.figure(figsize=(7.75,5))
ax1 = plt.subplot2grid((4,4), (0,2), colspan=2, rowspan=2)
ax2 = plt.subplot2grid((4,4), (0,0), colspan=2, rowspan=2)
ax11 = plt.subplot2grid((4,4), (2,0), colspan=1, rowspan=1)
ax12 = plt.subplot2grid((4,4), (3,0), colspan=1, rowspan=1)
ax21 = plt.subplot2grid((4,4),(2,1), colspan=1,rowspan=1)
ax22 = plt.subplot2grid((4,4),(3,1), colspan=1,rowspan=1)
ax31 = plt.subplot2grid((4,4),(2,2), colspan=1,rowspan=1)
ax32 = plt.subplot2grid((4,4),(3,2), colspan=1,rowspan=1)
ax41 = plt.subplot2grid((4,4),(2,3), colspan=1,rowspan=1)
ax42 = plt.subplot2grid((4,4),(3,3), colspan=1,rowspan=1)

# maps
ax1.imshow(cak_map_cropped, cmap = 'gray', aspect = 'equal', clim = (0.4,5.5))
ax2.imshow(ha_map_cropped, cmap = 'gray', aspect = 'equal')
...
ax2.set_xlabel('x [arcsec]', fontdict = font)
ax2.set_xticks(xtick_pos)
ax2.set_xticklabels(xtick_lab, fontdict=font)

# Vertical axis label
#f.text(0.001,0.46,r'I$_{mean}$ [10$^{-5}$ erg cm$^{-2}$ s$^{-1}$ Hz$^{-1}$ ster$^{-1}$]', fontdict=font, rotation = 90)
# Horizontal Axis label
f.text(0.5,0.005,r'$\mathrm{\Delta \lambda}$ [$\mathrm{\AA}$]', fontdict=font)
ax1.text(1,2, 'Ca II K $\mathrm{\lambda}$$-$integrated', color = 'white', fontdict = font, alpha = 0.7)
ax2.text(1,2,  'H$\mathrm{\\alpha}$ $\mathrm{\lambda}$$-$integrated', color = 'white', fontdict = font, alpha = 0.7)
...
for fn in range(len(file_search(fibdir,'crispex*3950*2018*.csav'))/2):
    ...
    # temperature along the fibril
    #f_temp = np.zeros((f_l),dtype=float)
    #for i in range (f_l-1):
     #   f_temp[i] = temp[int(f_y[i]-ymin), int(f_x[i]-xmin)]
    ...
    ax31.set_xticks(xtick_pos_cak)
    ax31.set_xlim(cak_xlim)
    ax31.tick_params(axis ='both', labelsize =8)
    ax32.set_xticks(xtick_pos_ha)
    ax32.set_xlim(ha_xlim)
    ax32.tick_params(axis ='both', labelsize =8)
    ax41.set_xticks(xtick_pos_cak)
    ax41.set_xlim(cak_xlim)
    ax41.tick_params(axis ='both', labelsize =8)
    ax21.set_xticks(xtick_pos_cak)
    ax21.set_xlim(cak_xlim)
    ax21.tick_params(axis ='both', labelsize =8)
    ax22.set_xticks(xtick_pos_ha)
    ax22.set_xlim(ha_xlim)
    ax22.tick_params(axis ='both', labelsize =8)
    ax42.set_xticks(xtick_pos_ha)
    ax42.set_xlim(ha_xlim)
    ax42.tick_params(axis ='both', labelsize =8)

    ax11.set_ylabel(r'$\mathrm{\bar{I}_{CaK}/I_c}$ [%]', fontdict=font)
    ax12.set_ylabel(r'$\mathrm{\bar{I}_{H\alpha}/I_c}$ [%]', fontdict=font)

    # yaxis ticks
    cak_ylim = [8,18.5]
    ha_ylim = [16.5,82]
    ax11.set_ylim(cak_ylim)
    ax21.set_ylim(cak_ylim)
    ax31.set_ylim(cak_ylim)
    ax41.set_ylim(cak_ylim)
    ax12.set_ylim(ha_ylim)
    ax22.set_ylim(ha_ylim)
    ax32.set_ylim(ha_ylim)
    ax42.set_ylim(ha_ylim)
    ...
    if (2*fn==cat_indx[0] or 2*fn==cat_indx[1] or 2*fn==cat_indx[2] or 2*fn==cat_indx[3]):
        ...
        cat_lab = ['(a)','(b)','(c)','(d)'] # category labels
        ...
        if (2*fn == cat_indx[0]):
            ...
            ax11.annotate(cat_lab[0], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            ax12.annotate(cat_lab[0], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            # sample labels
            ax1.text(f_x_pts[-1]-xmin-18, f_y_pts[-1]-ymin, cat_lab[0], alpha = 0.5, color = 'red')
        

            
        if (2*fn == cat_indx[1]):
            ax21.plot(cak_wav[cak_i:cak_n], f_Itot, color = 'red')
            ax21.plot(cak_wav[cak_i:cak_n], b_Itot, color = 'gray')
            ax22.plot(ha_wav[ha_i:ha_n], f_Itot_h, color = 'red')
            ax22.plot(ha_wav[ha_i:ha_n], b_Itot_h, color = 'gray')
            ax21.annotate(cat_lab[1], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            ax22.annotate(cat_lab[1], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            # sample labels
            ax1.text(f_x_pts[-1]-xmin-18, f_y_pts[-1]-ymin, cat_lab[1], alpha = 0.5, color = 'red')
            
        if (2*fn == cat_indx[2]):
            ax31.plot(cak_wav[cak_i:cak_n], f_Itot, color = 'red')
            ax31.plot(cak_wav[cak_i:cak_n], b_Itot, color = 'gray')
            ax32.plot(ha_wav[ha_i:ha_n], f_Itot_h, color = 'red')
            ax32.plot(ha_wav[ha_i:ha_n], b_Itot_h, color = 'gray')
            ax31.annotate(cat_lab[2], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            ax32.annotate(cat_lab[2], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            # sample labels
            ax1.text(f_x_pts[-1]-xmin-18, f_y_pts[-1]-ymin, cat_lab[2], alpha = 0.5, color = 'red')
            
        if (2*fn == cat_indx[3]):
            ax41.plot(cak_wav[cak_i:cak_n], f_Itot, color = 'red')
            ax41.plot(cak_wav[cak_i:cak_n], b_Itot, color = 'gray')
            ax42.plot(ha_wav[ha_i:ha_n], f_Itot_h, color = 'red')
            ax42.plot(ha_wav[ha_i:ha_n], b_Itot_h, color = 'gray')
            ax41.annotate(cat_lab[3], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            ax42.annotate(cat_lab[3], xy=(0.02, 0.87), xycoords='axes fraction', color = 'red', size = 8)
            # sample labels
            ax1.text(f_x_pts[-1]-xmin-18, f_y_pts[-1]-ymin, cat_lab[3], alpha = 0.5, color = 'red')
                
    else:
        lwidth = 1.25 # width of the lines
        alp = 0.3 #transparency of the plotted lines

        ax1.plot(f_x_pts-xmin,f_y_pts-ymin, color = 'white', linewidth = lwidth, alpha = alp, linestyle = ':')
        #ax2.plot(f_x_pts-xmin,f_y_pts-ymin, color = 'white', linewidth = lwidth, alpha = alp, linestyle = ':')
...
ax21.axvline(x = cak_wav[6], linestyle = ':', color = 'black', linewidth = 0.5)
ax21.axvline(x = cak_wav[14], linestyle = ':', color = 'black', linewidth = 0.5)
ax22.axvline(x = ha_wav[4], linestyle = ':', color = 'black', linewidth = 0.5)
ax22.axvline(x = ha_wav[10], linestyle = ':', color = 'black', linewidth = 0.5)

ax31.axvline(x = cak_wav[6], linestyle = ':', color = 'black', linewidth = 0.5)
ax31.axvline(x = cak_wav[14], linestyle = ':', color = 'black', linewidth = 0.5)
ax32.axvline(x = ha_wav[4], linestyle = ':', color = 'black', linewidth = 0.5)
ax32.axvline(x = ha_wav[10], linestyle = ':', color = 'black', linewidth = 0.5)

ax41.axvline(x = cak_wav[6], linestyle = ':', color = 'black', linewidth = 0.5)
ax41.axvline(x = cak_wav[14], linestyle = ':', color = 'black', linewidth = 0.5)
ax42.axvline(x = ha_wav[4], linestyle = ':', color = 'black', linewidth = 0.5)
ax42.axvline(x = ha_wav[10], linestyle = ':', color = 'black', linewidth = 0.5)

        
f.set_tight_layout(True)
plt.show()
plt.savefig(outdir+'category2_updated.pdf', quality = 100)
print 'file save to: '+ outdir+'category2_updated_with_lines.pdf'
