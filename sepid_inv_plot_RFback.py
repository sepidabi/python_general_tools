...
f_mRF = sp.model('/home/seki2695/INV/stic/fb/f'+mode+'_atmosout_cycle'+cyc+'_'+index+'.nc')
f_rf = sp.profile('/home/seki2695/INV/stic/fb/fRF_synthetic_cycle2_'+index+'.nc')
# backgraound
...
b_rf = sp.profile('/home/seki2695/INV/stic/bg/bRF_synthetic_cycle2_'+index+'.nc')
...
if plot_mode == 2:
    ...
    for what in range(2):
        if what == 0:
            ...
            outname = outdir+'inv_RF_'+index+'_'+mode+'_cycle'+cyc+'.pdf'
            temp_color_map = 'afmhot'
            marker_color = 'red'
        else:
            # background RF
            rf_t = b_rf.rf[0,0,:,0,:,0:143,0]
            rf_vlos = b_rf.rf[0,0,:,1,:,0:143,0]
            rf_vmt = b_rf.rf[0,0,:,2,:,0:143,0]
            outname = outdir+'inv_RF_'+index+'_'+mode+'_cycle'+cyc+'bg.pdf'
            temp_color_map = 'bone'
            marker_color = 'blue'
        ...
        for pp in range(gg):
            ...
            ax23.plot(f_vlosRF.squeeze()*1e-5,ax23y, 'k-',color = 'black',linewidth = 1., label = r'v$_{\rm f}$')
            ax23.plot(b_vlosRF.squeeze()*1e-5,ax23y, 'k-',color = 'gray',linewidth = 1., label = r'v$_{\rm b}$')
            ...
            ax33.plot(f_vmtRF.squeeze()*1e-5,ax33y, 'k-',color = 'black',linewidth = 1., label = r'v$_{\rm f}$')
            ...
            if pp==0:
                leg = ax1.legend( loc='best',
                           ncol=5, mode="expand", borderaxespad=0., fontsize = 8.5,
...
if plot_mode ==3:
    ...
    for what in range(2):
        ...
        leg = ax12.legend(loc = 'lower left',
        ...
        ax3.set_xlabel(r'pixel along the path', fontdict=font)
        ax3.set_ylabel(r'log($\mathrm{\tau_{500}}$)', fontdict=font)
        ax3.set_yticks(ytick_pos)
        ax3.set_yticklabels(ytick_lab.astype(int), fontdict=font)
