...
#for fn in range(10):
    ...
    for pp in range(np.min([f_slab_h.shape[1], b_slab_h.shape[1]])):
        ...
        if (f_k2v > f_slab[f_k2v_pos-1, pp] and
            ...
            if(b_k2v > b_slab[b_k2v_pos-1, pp] and
               ...
               b_k2r > b_slab[b_k2r_pos+1, pp]
                ...
                # K2 Asymmetry
                # 22222222222
                fI_k2v, fI_k2r = np.append(fI_k2v, 100*f_k2v/Inorm), np.append(fI_k2r, 100*f_k2r/Inorm)
                bI_k2v, bI_k2r = np.append(bI_k2v, 100*b_k2v/Inorm), np.append(bI_k2r, 100*b_k2r/Inorm)
        ...
        # K1 minimums
        if (f_k1v < f_slab[f_k1v_pos-1, pp] and
            ...
            f_k1ntot+=1
            # double-peaked in both fb and bg
            if(b_k1v < b_slab[b_k1v_pos-1, pp] and
               ...
               b_k1r < b_slab[b_k1r_pos+1, pp]
                ...
                # K1 Asymmetry
                # 111111111111
                fI_k1v, fI_k1r = np.append(fI_k1v, 100*f_k1v/Inorm), np.append(fI_k1r, 100*f_k1r/Inorm)
                bI_k1v, bI_k1r = np.append(bI_k1v, 100*b_k1v/Inorm), np.append(bI_k1r, 100*b_k1r/Inorm)
                ...
                print('wierd K1 profile :/','fibril no. ', fn, ' - pixel ', pp)
                #print('============')
...
# Set labels on joint
ax_joint.set_xlabel(r'$R \mathrm{_{A,f}}$', fontdict = font)
ax_joint.set_ylabel(r'$R \mathrm{_{A,b}}$', fontdict = font)
sns.kdeplot(f_A,b_A, cmap = 'Reds', ax = ax_joint, shade = True)
sns.kdeplot(f_A, ax = ax_marg_x, shade=True, color = 'orangered')
sns.kdeplot(b_A, ax = ax_marg_y, shade=True, color = 'orangered', vertical = True)
...
ax_joint.tick_params(labelsize = fontsize-1)
ax_marg_x.tick_params(labelsize = fontsize)
ax_marg_y.tick_params(labelsize = fontsize)

pearsonr = str(np.round(stat.pearsonr(f_A, b_A)[0],decimals = 2))
pearsonp = str(np.int(stat.pearsonr(f_ksep, b_ksep)[1]))
ax_joint.annotate('c) pearsonr = ' + pearsonr,xy=(0.05,0.92), fontsize = fontsize-1, xycoords='axes fraction')

# extras overplotted lines
xx = np.linspace(xmin, xmax, 10)
ax_joint.plot(xx, xx, linestyle = ':', color = 'darkred', linewidth = 0.75)
ax_joint.text(0.15, 0.18, 'y = x', fontsize = fontsize-2, rotation = 45, color = 'darkred', va = 'bottom', ha = 'left')


plt.tight_layout()
plt.subplots_adjust(left = left+0.02,
                    bottom = bottom,
                    right = 1.,
                    top = top,
                    wspace = wspace,
...
plt.show()
fig.savefig(outdir + 'k2_asymmetry_modified.pdf', dpi = 1000)



#################
# the K2-SEAPRATION plot
#################
plt.close('all')
fig = plt.figure(figsize = (fsize_x,fsize_y))

xmin, xmax = np.min(f_ksep), np.max(f_ksep)
ymin, ymax = np.min(b_ksep), np.max(b_ksep)
dlambda = abs(w_ck[10]-w_ck[9])
...
bins_joint = abs(xmax-xmin)/dlambda
bins_marg = np.int(np.round(bins_joint))

ax_joint.hist2d(f_ksep, b_ksep, cmap = 'Reds', vmin = 0, vmax = 1700)
ax_marg_x.hist(f_ksep, color = 'orangered', rwidth = 0.95, alpha = 0.5)
ax_marg_y.hist(b_ksep,orientation="horizontal", color = 'orangered', alpha = 0.5, rwidth = 0.95)
...
# Set labels on joint
ax_joint.set_xlabel(r'$\Delta \lambda \mathrm{_{K2,f}}$ [$\mathrm{\AA}$]', fontdict = font)
ax_joint.set_ylabel(r'$\Delta \lambda \mathrm{_{K2,b}}$ [$\mathrm{\AA}$]', fontdict = font)
...
pearsonr = str(np.round(stat.pearsonr(f_ksep, b_ksep)[0],decimals = 2))
pearsonp = str(np.int(stat.pearsonr(f_ksep, b_ksep)[1]))
ax_joint.annotate('e) pearsonr = ' + pearsonr,xy=(0.05,0.92), fontsize = fontsize-1, xycoords='axes fraction')

# extras overplotted lines
xx = np.linspace(xmin, xmax, 10)
ax_joint.plot(xx, xx, linestyle = ':', color = 'darkred', linewidth = 0.75)
ax_joint.text(0.8, 0.85, 'y = x', fontsize = fontsize-2, rotation = 45, color = 'darkred', va = 'bottom', ha = 'left')
...
plt.show()
fig.savefig(outdir + 'k2_sep_modified.pdf', dpi = 1000)
...
#################
# the K1-SEAPRATION plot
#################
plt.close('all')
fig = plt.figure(figsize = (fsize_x,fsize_y))

xmin, xmax = np.min(f_k1sep), np.max(f_k1sep)
ymin, ymax = xmin, xmax #np.min(b_k1sep), np.max(b_k1sep)
dlambda = abs(w_ck[10]-w_ck[9])
...
ax_joint.hist2d(f_k1sep, b_k1sep, cmap = 'Reds', vmin = 0, vmax = 2000)
ax_marg_x.hist(f_k1sep, color = 'orangered', rwidth = 0.95, alpha = 0.5)
ax_marg_y.hist(b_k1sep,orientation="horizontal", color = 'orangered', alpha = 0.5, rwidth = 0.95)
...
# Set labels on joint
ax_joint.set_xlabel(r'$\Delta \lambda \mathrm{_{K1,f}}$ [$\mathrm{\AA}$]', fontdict = font)
ax_joint.set_ylabel(r'$\Delta \lambda \mathrm{_{K1,b}}$ [$\mathrm{\AA}$]', fontdict = font)
ax_joint.tick_params(labelsize = fontsize)
ax_marg_x.tick_params(labelsize = fontsize)
ax_marg_y.tick_params(labelsize = fontsize)
...
pearsonr = str(np.round(stat.pearsonr(f_k1sep, b_k1sep)[0],decimals = 2))
pearsonp = str(np.int(stat.pearsonr(f_k1sep, b_k1sep)[1]))
ax_joint.annotate('d) pearsonr = ' + pearsonr,xy=(0.05,0.92), fontsize = fontsize-1, xycoords='axes fraction')

# extras overplotted lines
xx = np.linspace(xmin, xmax, 10)
ax_joint.plot(xx, xx, linestyle = ':', color = 'darkred', linewidth = 0.75)
ax_joint.text(0.45,0.5, 'y = x', fontsize = fontsize-2, rotation = 45, color = 'darkred', va = 'bottom', ha = 'left')
...
plt.show()
fig.savefig(outdir + 'k1_sep_modified.pdf', dpi = 1000)


################
# the K2 intensity plot
################

x = np.mean([fI_k2v,fI_k2r], axis = 0, dtype=float)
y = np.mean([bI_k2v,bI_k2r], axis = 0, dtype=float)

plt.close('all')
fig = plt.figure(figsize = (fsize_x,fsize_y))

xmin, xmax = np.min(x), 26
...
# Set labels on joint
ax_joint.set_xlabel(r'$I\mathrm{_{K2,f}}$ / $I\mathrm{_{cont}}$ [%]', fontdict = font)
ax_joint.set_ylabel(r'$I\mathrm{_{K2,b}}$ / $I\mathrm{_{cont}}$ [%]', fontdict = font)
...
ax_joint.annotate('b) pearsonr = ' + pearsonr,xy=(0.05,0.92), fontsize = fontsize-1, xycoords='axes fraction')
ax_joint.text(22,23, 'y = x', fontsize = fontsize-2, rotation = 45, color = 'darkred', va = 'bottom', ha = 'left')
...
plt.show()
fig.savefig(outdir + 'I_k2.pdf', dpi = 1000)


################
# the K1 intensity plot
################

x = np.mean([fI_k1v,fI_k1r], axis = 0, dtype=float)
y = np.mean([bI_k1v,bI_k1r], axis = 0, dtype=float)

plt.close('all')
fig = plt.figure(figsize = (fsize_x,fsize_y))

xmin, xmax = np.min(x), 16
...
# Set labels on joint
ax_joint.set_xlabel(r'$I\mathrm{_{K1,f}}$ / $I\mathrm{_{cont}}$ [%]', fontdict = font)
ax_joint.set_ylabel(r'$I\mathrm{_{K1,b}}$ / $I\mathrm{_{cont}}$ [%]', fontdict = font)
sns.kdeplot(x,y, cmap = 'Reds', ax = ax_joint, shade = True)
sns.kdeplot(x, ax = ax_marg_x, shade=True, color = 'orangered')
sns.kdeplot(y, ax = ax_marg_y, shade=True, color = 'orangered', vertical = True)
...
ax_joint.text(13.25,14, 'y = x', fontsize = fontsize-2, rotation = 45, color = 'darkred', va = 'bottom', ha = 'left')
...
plt.show()
fig.savefig(outdir + 'I_k1.pdf', dpi = 1000)
