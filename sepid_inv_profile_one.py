...
b_i = sp.profile(resdir+'bg/b'+mode+'_observed'+'_'+index+'.nc')
...
ffile_fe = file_search(savedir, 'f_obs6302_'+index+'.fits')[0]
ffile_ca8 = file_search(savedir, 'f_obs8542_'+index+'.fits')[0]
ffile_ck = file_search(savedir, 'f_obs3950_'+index+'.fits')[0]
bfile_fe = file_search(savedir, 'b_obs6302_'+index+'.fits')[0]
bfile_ca8 = file_search(savedir, 'b_obs8542_'+index+'.fits')[0]
bfile_ck = file_search(savedir, 'b_obs3950_'+index+'.fits')[0]
#index = ffile_fe
finit = 0
binit = 0
...
fibril = mf.readfits(savedir + ffile_fe)
dark = mf.readfits(savedir + bfile_fe)
...
ax11.plot(np.mean(f_syn, axis = 0), color='r',linewidth = 1.25)
